...
//        intBuffer.put(receivedata);
        ...
        if(revData[0] == 0x55 && revData[1] ==0x02){
            if(revData[2]==0x02){  //28字节报文，机器人位置，小端数据
                if(isCRCOk(revData,28)){  //CRC校验
                    Double positionx= BytesUtil.parseDouble( BytesUtil.getNInts(revData,3,8));
                ...
                if (isCRCOk(revData, 15)) {  //CRC校验
                    switch (revData[2]){
                        ...
                        case 0x01:  //PM_Hum_Smoke_Volt
                            int pm25= BytesUtil.parseUnint16( BytesUtil.getNInts(revData,3,2));
                            int pm10=BytesUtil.parseUnint16( BytesUtil.getNInts(revData,5,2));
//                            int zero= revData[7]; //零上or零下
                            int temperature= revData[8]; //绝对温度
                            int humidity=revData[9] ; //湿度
                            int smoke=BytesUtil.parseUnint16( BytesUtil.getNInts(revData,10,2)); //
